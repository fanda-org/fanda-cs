...
namespace DataTables.Queryable
{
    ...
    internal class PagedList<T> : List<T>, IPagedList<T>
    {
        ...
        internal PagedList(IQueryable<T> queryable, SieveModel sieveModel) : base()
        {
            // pagination is on
            if (sieveModel.PageSize > 0)
            {
                int skipCount = ((int)sieveModel.Page - 1) * (int)sieveModel.PageSize;
                int takeCount = (int)sieveModel.PageSize;

                TotalCount = queryable.Count();
                PageNumber = (int)sieveModel.Page;
                PageSize = (int)sieveModel.PageSize;
