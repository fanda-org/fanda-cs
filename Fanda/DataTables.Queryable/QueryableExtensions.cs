...
namespace DataTables.Queryable
{
    ...
    public static class QueryableExtensions
    {
        ...
        private static readonly MethodInfo Bool_Equals = typeof(EF).GetMethod("Property").MakeGenericMethod(typeof(bool));
        /// <summary>
        ...
        private static Expression<Func<T, bool>> BuildStringContainsPredicate<T>(string propertyName, string stringConstant, bool caseInsensitive)
        {
            ...
            {
                if (stringConstant.ToLower() == "true" || stringConstant.ToLower() == "false")
                {
                    exp = Expression.Call(Bool_Equals, parameterExp, Expression.Constant(propertyName));
                    var boolValue = System.Convert.ToBoolean(stringConstant);
                    BinaryExpression compareExpression = Expression.MakeBinary(ExpressionType.Equal, exp, Expression.Constant(boolValue));
