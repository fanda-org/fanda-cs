namespace Fanda.Authentication.Controllers
{
    ...
    public class ActionsController : BaseController
    {
        ...
        public ActionsController(IActionRepository repository)
        {
        ...
        public async Task<IActionResult> GetById([Required, FromRoute] Guid id, [FromQuery] bool include)
        {
            try
            {
                var action = await repository.GetByIdAsync(id, include);
                if (action != null)
                {
                    return Ok(DataResponse<ActionDto>.Succeeded(action));
                }
                return NotFound(DataResponse.Failure($"Action id '{id}' not found"));
            }
            catch (Exception ex)
            {
                if (ex is BadRequestException || ex is ArgumentNullException || ex is ArgumentException)
                {
                    return BadRequest(DataResponse.Failure("Invalid action id"));
                }
                else if (ex is NotFoundException)
                {
                    return NotFound(DataResponse.Failure("Action not found"));
                }
                else
                {
                    return InternalServerError(DataResponse.Failure(ex.Message));
        ...
        [ProducesResponseType(StatusCodes.Status500InternalServerError)]
        public async Task<IActionResult> Create(ActionDto model)
        {
            try
            {
                #region Validation
                ...
                {
                    var action = await repository.CreateAsync(model);
                    return CreatedAtAction(nameof(GetById), new { id = model.Id, include = false },
                        DataResponse<ActionDto>.Succeeded(action));
                }
                else
                {
        ...
        [ProducesResponseType(StatusCodes.Status401Unauthorized)]
        [ProducesResponseType(StatusCodes.Status500InternalServerError)]
        public async Task<IActionResult> Update(Guid id, ActionDto model)
        {
            try
            {
                if (id != model.Id)
                {
                    return BadRequest(DataResponse.Failure("Action id mismatch"));
                }
                #region Validation
                var validationResult = await repository.ValidateAsync(model);
                #endregion
